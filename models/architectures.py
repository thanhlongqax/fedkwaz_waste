...
# ─── Client 2: garbage dataset (│MobileV3 ) ─────────────────────
class MobileNetV3WasteDetector(WasteClientModel):
    """
    Mô hình tối giản cho Client 2 
    Hardware: Raspberry Pi 4, garbage RGBD
    ...
    def forward(self, x: torch.Tensor, depth: Optional[torch.Tensor] = None) -> Dict:
        ...
        spatial_feat = F.adaptive_avg_pool2d(spatial_feat, (8, 8)).flatten(1)
        return {
            "logits": logits,
            "global_feat": global_feat,
            "spatial_feat": F.adaptive_avg_pool2d(spatial_feat, 1).flatten(1),
...
# ───  ────────────────────────────────────────────────────────────────────────
# ─── Client 3: DenseNet121WasteDetector (Raspberry Pi - recyclable-household-waste) ────────────────────────
class DenseNet121WasteDetector(WasteClientModel):
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        return {
            "logits": logits,
            "global_feat": global_feat,
            "spatial_feat": F.adaptive_avg_pool2d(
                spatial_feat,
                1
            ).flatten(1),
            "raw_feat": feats["feat_map"]
...
def build_client_model(
    ...
    **kwargs
    ...
    model_map = {
        ...
        "densenet121": DenseNet121WasteDetector,
        "proxy": ProxyModel,
    }
